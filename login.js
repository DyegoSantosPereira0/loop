...
submitBtn.addEventListener('click', async () => {
  ...
  if(!username || !password){ messageDiv.textContent = 'Preencha todos os campos'; return; }
  ...
  if(isLogin){
    ...
    try{
      ...
      if(res.ok){
        ...
        window.location.href = 'index.html'; // abrir app
      } else {
        messageDiv.textContent = data.message || 'Erro no login';
      }
    }catch(err){ messageDiv.textContent = 'Erro de conexão'; }
  } else {
    // Registrar
    try{
